...
def mail():
    top=Toplevel()
    top.title("Mailing window")
    top.iconbitmap("download.ico")
    top.geometry("500x250")

    lbl=Label(top,text="Reciever's mail ID")
    lbl.grid(row=0,column=0,padx=10,pady=10)

    enter=Entry(top,width=50,bd=10,relief=SUNKEN)
    enter.grid(row=0,column=1,padx=20,pady=50)

    button=Button(top,text="Send",command=send)
...
def recomendation(a):
    ...
    label = Label(root, text=text, bg='white',relief=GROOVE, font=30, anchor='nw', justify='left', bd=10)
    label.place(x= q,y=w, relwidth=0.15, relheight=0.45)
...
mailing_menu=Menu(my_menu,tearoff=False)
my_menu.add_cascade(label="Mailing", menu=mailing_menu)
mailing_menu.add_command(label="Mail",command=mail)
